...
Acceptance: 91 Linux-runnable non-GUI tests pass; Ruff format/check, compileall, CLI config redaction, and Git whitespace checks pass. Durable final reconciliation verifies all remote artifact hashes/sizes, missing server files reset and retry safely, generated files and upload responses are integrity-checked, and encoded MP4 output is post-probed for resolution/FPS preservation. SQLite has Windows/POSIX single-writer ownership with non-mutating read-only diagnostics, preset changes invalidate only true dependents, strict meeting responses cannot redirect uploads, Bucharest historical/DST offsets remain Laravel-compatible, Retry-After HTTP dates are honored, and non-loopback token transport requires HTTPS. Runtime binaries, models, caches, state, work files, and credentials are ignored. Windows ROCm/AMF hardware behavior and distributable packaging remain Stage 4/5 acceptance work.
